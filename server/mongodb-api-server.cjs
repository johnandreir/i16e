...
async function attemptReconnection() {
  ...
  try {
    if (connectionRetryCount < MAX_RETRY_ATTEMPTS) {
      ...
      console.log(`🔄 Attempting to reconnect to MongoDB in ${backoffDelay/1000} seconds... (attempt ${connectionRetryCount + 1}/${MAX_RETRY_ATTEMPTS})`);
      ...
      console.log('⚠️ Maximum reconnection attempts reached. Will retry again in 30 seconds...');
  ...
  if (entityType === 'squad') {
    ...
    if (standardized.teamID && typeof standardized.teamID !== 'object') {
      try {
        ...
        console.warn(`❌ Failed to convert teamID to ObjectId: ${err.message}`);
  ...
  if (entityType === 'dpe') {
    ...
    if (standardized.squadID && typeof standardized.squadID !== 'object') {
      try {
        ...
        console.warn(`❌ Failed to convert squadID to ObjectId: ${err.message}`);
        // Keep as is if conversion fails
      }
    }
  }
        ...
        if (workflowsResponse.ok) {
          ...
          try {
            ...
            if (fs.existsSync(workflowsDir)) {
              ...
              console.log(`📊 Found ${workflowFiles.length} workflow files (${activeCount} active)`);
            } else {
              // Fallback to known correct values if directory doesn't exist
              console.log('⚠️ N8N workflows directory not found - using known correct values');
              workflowStatus = {
                reachable: true,
            ...
            console.log('⚠️ Error reading workflow files:', fsError.message);
            workflowStatus = {
              reachable: true,
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (!isConnected || !db) {
        ...
        const isDockerEnv = process.env.NODE_ENV === 'production' || process.env.IN_DOCKER === 'true';
        if (isDockerEnv) {
          console.log('🔄 Docker environment detected, attempting immediate reconnection...');
          await connectToMongoDB(0);
          
          // If reconnection was successful, continue with the operation
          if (isConnected && db) {
            console.log('✅ Reconnection successful, continuing with operation');
          } else {
...
app.post('/api/team', async (req, res) => {
  try {
    ...
    console.log('🔍 Team data to be inserted:', JSON.stringify(teamData, (key, value) => {
      return value instanceof Date ? value.toISOString() : value;
    }, 2));
    console.log('🔍 Team data types:', {
    ...
    const insertOperation = async (collection) => {
      try {
        ...
        console.error('❌ Insert operation error details:', error);
        if (error.code === 121) { // Document validation error code
...
app.put('/api/squad/:id', async (req, res) => {
  ...
  console.log('📝 Squad update data after standardization:', updateData);

  try {
    console.log('🔄 Executing Squad update operation with standardized fields...');
    ...
    if (duplicateCheck) {
      console.log('❌ Duplicate squad with same name and teamID:', name, teamID);
      return res.status(409).json({ 
    ...
    if (!result) {
      console.log('❌ Squad update failed but no error was thrown');
    ...
    if (error.name === 'MongoServerError') {
      if (error.code === 121) { // Document validation error
        ...
        console.log('🔍 Validation error details:', error.errInfo);
...
app.put('/api/dpe/:id', async (req, res) => {
  ...
  console.log('📝 DPE update data after standardization:', updateData);

  try {
    console.log('🔄 Executing DPE update operation with standardized fields...');
    ...
    if (!squadExists) {
      console.log('❌ Referenced squad does not exist:', squadID);
      return res.status(400).json({ 
    ...
    if (duplicateCheck) {
      console.log('❌ Duplicate DPE with same name and squadID:', name, squadID);
      return res.status(409).json({ 
        error: 'Duplicate DPE', 
        code: 'DUPLICATE_DPE',
    ...
    if (!result) {
      console.log('❌ DPE update failed but no error was thrown');
      return res.status(500).json({ 
        error: 'Update failed', 
        code: 'UPDATE_FAILED',
...
async function startServer() {
  ...
  console.log(`🔧 In Docker: ${process.env.IN_DOCKER === 'true' ? 'Yes' : 'No'}`);
  ...
  if (process.env.IN_DOCKER === 'true') {
    ...
    console.log(`🔄 Docker environment detected, waiting ${initialDelay/1000} seconds before connecting to MongoDB...`);
